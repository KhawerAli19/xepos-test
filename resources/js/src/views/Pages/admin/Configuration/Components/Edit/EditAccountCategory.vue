    ...
    <div class="row">
        <div class="col-md-12">
            <div class="titleBox py-4">
                <h3 class="mb-0 achivpFont mb-0 font-weight-bold"><router-link :to="{ name: 'admin.configuration.index' , params:{type:'account-category'} }" class="text-dark text-decoration-none"><i class="bi bi-chevron-left"></i>Edit Category</router-link></h3>
            </div>
        </div>
    </div>
                ...
                <form @submit.prevent="handleSubmit(Update)">
                    <div class="d-flex mb-3 flex-wrap align-items-center">
                        ...
                        <div class="fieldData col-md-6">
                             <validation-provider tag="div" rules="required" name="category" v-slot="{errors}" >
                            <input type="text" v-model="form.name" placeholder="Category Name" name="category" class="form-control pr-5 py-2 h-auto">
                    ...
                    <div class="d-flex mb-3 flex-wrap align-items-center">
                        <div class="labelTitle col-md-6">
                            <p class="mb-0 font-weight-bold">Color for category :</p>
                        </div>
                        <div class="fieldData col-md-6">
                             <validation-provider tag="div" rules="required" name="color" v-slot="{errors}" >
                            <input type="color" v-model="form.color" id="color" name="color" value="#ff0000">
                             <div class="text-danger">{{errors[0]}}</div>
                            </validation-provider>
                        </div>
                    </div>
...
export default {
    ...
    methods: {
        ...mapActions('admin', ['store' ,'getAll']),
           async fetch(page = 1) {
			let params = {
				route: route('admin.configuration.category.show',{id: this.$route.params.id}),
				 data: {
                    page
                }
			};
        ...
        async Update() {
            ...
            let params = {
                route: route('admin.configuration.category.update', {id: this.$route.params.id}),
                method: 'POST',
                data: fd,
            };
            try {
                ...
                if (data.status) {
                    ...
                    this.$router.push({ name: 'admin.configuration.index' , params:{type:'account-category'} });
